...
class VehicleData:
    ...
    def filter(self, rule: str = None, sort_type: str = "击杀") -> list:
        ...
        if sort_type.upper() in ["KPM"]:
            vehicle_list.sort(
                key=lambda x: (
                    x.get("stats").get("values").get("kills", 0)
    ...
    def search_vehicle(self, target_vehicle_name: str, sort_type: str = "击杀") -> list:
        ...
        vehicle_list.sort(
            key=lambda x: (
                x.get("stats").get("values").get(sort_type_dict.get(sort_type, "kills"), 0)
            ),
            reverse=True,
        )
        return vehicle_list
...
class BTRMatchesData:
    ...
    async def handle(self) -> list[dict]:
        ...
        for i, match in enumerate(self.btr_matches_data):
            ...
            try:
                map_name = re.findall(re.compile(r'<h2 class="map-name">(.*?)<small'), str(match))[
                    0
                ]
            except IndexError:
                try:
            ...
            # 服务器名
            server_name = (
                soup.select("div.match-info")[0].select("small.hidden-sm.hidden-xs")[0].text
            )
            ...
            for team_item in soup.select("div.match-teams")[0].select("div.team"):
                ...
                if team_name == "Team1":
                    ...
                    team_name = 0
                team_win = (
                    team_item.select("div.card-heading")[0].select("div.additional-info")[0].text
                )
                if team_win == "Won":
                    team_win = True
                else:
                ...
                for player in players:
                    ...
                    if player_kills.isdigit():
                        ...
                        player_kills = 0
                    player_deaths = (
                        player_stats[0].select("div.stat")[1].select("div.value")[0].text
                    )
                    ...
                    if player_headshots.isdigit():
                        ...
                        player_headshots = 0
                    player_headshots = (
                        f"{round(player_headshots / player_kills, 2) if player_kills != 0 else 0}%"
                    )
                    # 命中率序号不定
                    # 通过value来确定
                    player_accuracy = "0%"
