...
`ea_bindings.is_primary` 不在表层做 `UNIQUE(user_id) WHERE is_primary`，而是在 service 层保证「一个 user 至多一条 primary binding」。理由是 PostgreSQL 的 partial unique index 跨方言支持差，service 层做事务校验更稳。
...
1. 新建 `ea_bindings` 表，含 `persona_id UNIQUE` 索引。
2. 在 `users` 表新增 `username`、`local_password_hash`、`email`、`is_frozen` 字段（先 nullable）。
3. 把每个现有 user 的 `persona_id / display_name / avatar_url / encrypted_*` 复制到一条新 `ea_bindings` 记录，`is_primary=true`，`is_frozen=false`，`last_verified_at` 取 `last_login_at`。
4. 用 `persona_<persona_id>` 填充每个 user 的 `username`，置 `NOT NULL`、`UNIQUE`。
5. 把环境变量 `ADMIN_PERSONA_IDS`（从迁移执行时的 env 读取一次）内的 persona 对应 user `role` 设为 `admin`。
6. 删除 `users` 表中 `persona_id`、`display_name`、`avatar_url`、`encrypted_*` 字段及索引。

**第二步（代码切换）**：在同一次发布里，backend 代码切换到读 `ea_bindings`，停止读 `admin_persona_ids` env。

**回滚策略**：迁移记录原 user 字段值到 `ea_bindings`，回滚可由 binding 反向重建 `users.persona_id` 等字段（写 down revision）。在 staging 环境跑双向验证。
...
**风险三：persona_id UNIQUE 约束在迁移时可能冲突**。理论上现有 `users.persona_id` 已经 unique，迁移后应当无冲突，但若历史数据存在脏数据需先清理。迁移脚本第一步先 `SELECT persona_id, COUNT(*) FROM users GROUP BY persona_id HAVING COUNT(*)>1` 检查，发现冲突立即报错退出。
