...
- **WHEN** 任何写入路径试图在同一 user 下让两条 binding 同时持有 `is_primary=true`
- **THEN** PostgreSQL partial unique index `uq_ea_bindings_user_primary` 在事务提交时拒绝；service 层在创建新 binding 前必须先确认当前 user 无其他 primary，未来若新增「显式切换 primary」的写入路径，则该路径必须在同一事务内把原 primary 置 false 后再写入新 primary
